...
/**
 * This is a 1:1 translation of the algorithm on wikipedia:
 ...
 * return failure
 * 
 * function reconstruct_path(came_from,current) total_path := [current] while
 * current in came_from: current := came_from[current]
...
public class AStarAlgorithm {
	...
	/**
	 * If true the open and closed set are cloned each step of the A* path finding
	 * algorithm.
	 */
	boolean isSnapshotEnabled = true;

	/**
	 * Used for visualization of the single steps in the A* algorithm. No other
	 * relevance to the algorithm.
	 */
	...
	public List<AStarCell> getPath(AStarGrid grid, AStarCell start, AStarCell goal, boolean allowDiagonals) {
		...
		while (!openSet.isEmpty()) {
			...
			// note: we have a priorityqueue => for performance reasons we also remove the
			// item instead of removing it later (as suggested in the algorithm)
			...
			for (AStarCell neighbor : grid.getNeighbors(current, allowDiagonals)) {
				...
				if (!(containsNeighbor = openSet.contains(neighbor))
						|| Double.compare(tentativeScoreG, neighbor.g) < 0) {
	...
	private List<AStarCell> reconstructPath(AStarCell current) {
		...
		List<AStarCell> totalPath = new ArrayList<>(200); // arbitrary value, we'll most likely have more than 10 which
															// is default for java
	...
	/**
	 * Distance between a given cell and its neighbor. Used in the algorithm as
	 * distance calculation between the current cell and a neighbor. In our case we
	 * use the same distance which we use from the current cell to the goal.
	 */
	...
	/**
	 * Distance between two cells. We use the euclidian distance here. Used in the
	 * algorithm as distance calculation between a cell and the goal.
	 */
